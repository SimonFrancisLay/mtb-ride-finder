...
if not LOCATIONS:
    st.error("No locations found. Ensure 'config.yaml' is present and valid.")
    st.stop()
...
with st.sidebar:
    ...
    home_key = st.selectbox("Home / Start location", all_keys, index=default_home_idx,
                            format_func=lambda k: key_to_name.get(k, k),
                            help="Drive times are measured from here.")
...
depart_dt = depart_dt_raw.replace(minute=0, second=0, microsecond=0)
...
curve_extra_limit = 60  # minutes beyond baseline
...
def build_features(rows, exclude_key=None):
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        comps = r["components"]
        feats.append({"rank": idx, "name": r["name"], "score": r["score"], "lat": lat, "lon": lon, "radius": radius_m, "color": color,
                      "weather": comps["weather"], "trail": comps["trail"], "proximity": comps["proximity"], "terrain_fit": comps["terrain_fit"],
                      "secondary": comps["secondary"], "window": r["recommend_window"], "drive": r["drive_est_min"]})
    return feats
...
def build_features_out_of_range(rows, exclude_key=None):
    ...
    for r in rows:
        ...
        feats.append({"rank": "X", "name": r.get("name", "out-of-range"), "score": r.get("score", 0), "lat": lat, "lon": lon, "radius": radius_m,
                      "color": [128, 0, 128, 0], "weather": r.get("components",{}).get("weather", 0), "trail": r.get("components",{}).get("trail", 0),
                      "proximity": r.get("components",{}).get("proximity", 0), "terrain_fit": r.get("components",{}).get("terrain_fit", 0),
                      "secondary": r.get("components",{}).get("secondary", 0), "window": r.get("recommend_window", ""), "drive": r.get("drive_est_min", 0)})
    return feats
...
if not HAVE_PYDECK:
    ...
    if features or curve_pick or out_rows_for_map:
        ...
        layers = []
        if features:
            layers.append(pdk.Layer("ScatterplotLayer", data=features, get_position='[lon, lat]', get_radius="radius",
        ...
        if out_rows_for_map:
            ...
            layers.append(pdk.Layer("ScatterplotLayer", data=out_feats, get_position='[lon, lat]', get_radius="radius", filled=False,
                                    stroked=True, get_line_color=[128,0,128], line_width_min_pixels=2, pickable=True))
        if curve_pick is not None:
            clat, clon = loc_lookup.get(curve_pick["key"], (None, None))
            if clat is not None:
                tri = triangle_coords(clat, clon, size_m=5000)  # big triangle
                layers.append(pdk.Layer("PolygonLayer", data=[{"polygon": tri, "name": curve_pick["name"]}],
                                        get_polygon="polygon", get_fill_color=[255,100,0,220], get_line_color=[0,0,0,255], line_width_min_pixels=2, pickable=True))

        tooltip = {"html": ("<b>#{rank} {name}</b><br/>Score: <b>{score}</b><br/>"
        ...
        deck = pdk.Deck(layers=layers, initial_view_state=pdk.ViewState(latitude=avg_lat, longitude=avg_lon, zoom=6.8, pitch=0),
                        map_style=None, tooltip=tooltip)
        st.pydeck_chart(deck, use_container_width=True)

        with st.expander("Legend / colors"):
            st.markdown("- **#1** green • **#2** yellow • **#3** blue • **#4+** purple\n"
                        "- **Orange triangle** = curve ball (≤ +60 min beyond the #1 filtered drive)\n"
                        "- **Hollow purple circle** = outside current Max drive")
    else:
