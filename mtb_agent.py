...
def fetch_open_meteo(lat: float, lon: float, timezone: str, past_days: int = 7) -> dict:
    ...
    r = requests.get(base, params=params, timeout=20); r.raise_for_status(); return r.json()

def clamp(v, lo, hi): return max(lo, min(hi, v))

def map_range(x, a, b, c, d):
    if b == a: return (c + d)/2
    t = (x - a) / (b - a); t = clamp(t, 0, 1); return c + t*(d - c)
...
def score_proximity(typical_range: Tuple[int,int], depart_hh: int, max_drive: int) -> float:
    base = sum(typical_range)/2; est = base * rush_hour_multiplier(depart_hh)
    if est > max_drive: return 0.0
    return map_range(est, 0, max_drive, 100, 0)
...
def score_location(loc: Location, depart_dt: dt.datetime, duration_h: float, terrain_bias: float, max_drive: int, season: str, tech_bias: float = None):
    ...
    prox_s = score_proximity(loc.drive_min_typical, depart_dt.hour, max_drive)
    ...
    reason = assemble_reason(loc, wx_s, tr_s, prox_s, terr_s, sec_s, depart_dt, duration_h, drive_est, precip7, wind_mean, gust_mean, rain_prob)
    return {"key": loc.key,"name": loc.name,"score": round(total, 1),
...
def render_output(results: List[Dict], top_n: int = 5):
    ...
    for i, r in enumerate(sorted(results, key=lambda x: x["score"], reverse=True)[:top_n], start=1):
        ...
        for n in r["notes"][:3]: print(f"   • {n}")
        if len(r["notes"]) > 3: print("   • …"); print()
...
def main():
    ...
    for loc in locs:
        try:
            ...
            results.append({"key": loc.key,"name": loc.name,"score": 0.0,"components":{"weather":0,"trail":0,"proximity":0,"terrain_fit":0,"secondary":0},
    ...
    print(f"Start: {start_label} | Depart: {depart_dt:%H:%M} | Max drive: {max_drive} min | Elevation bias (distance↔hills): {terrain_bias:+.2f} | Tech bias (chilled↔gnar): {tech_bias:+.2f} | Duration: {duration_h} h | Season: {season}")
